...
Bool_t Ali_make_tracklets_from_digits::UserNotify() {
    ...
    for (Int_t i_entry = 0; i_entry < TRD_align_array->GetEntries(); i_entry++) {
        ...
        // \n",i_entry,name,sname.Length(),sector_name.Data(),stack_name.Data(),layer_name.Data(),detector); TM_TRD_align.Print();
        if (length > 8) {
...
void Ali_make_tracklets_from_digits::UserCreateOutputObjects() {
    ...
    h_ADC = new TH1D("h_ADC", "h_ADC", 200, 0, 4000);
...
void Ali_make_tracklets_from_digits::UserExec(Option_t*) {
    ...
    if (fMCEvent) {
        ...
        for (Int_t i_MC_track = 0; i_MC_track < N_MC_tracks; i_MC_track++) {
            ...
            // printf("MC_PDG_code: %d \n",MC_PDG_code);
            if (MC_PDG_code < 10000 && !(MC_PDG_code == 990 || MC_PDG_code == 9990 || MC_PDG_code == 110 || MC_PDG_code == 41 ||
                ...
                // printf("i_MC_track: %d, PDG_code: %d, mass: %4.3f, momentum: {%4.3f, %4.3f, %4.3f}, vertex: {%4.3f, %4.3f, %4.3f}, 1st
                // mother: %d, 2nd mother: %d, N_daugthers: %d
    ...
    TClonesArray* Arr_TOF_Hits = fESD->GetESDTOFHits();
    Int_t NTOFcls = fESD->GetNTOFclusters();
    for (Int_t ntof = 0; ntof < NTOFcls; ntof++) {
        AliESDTOFCluster* thiscluster = (AliESDTOFCluster*)Arr_TOF_Cluster->At(ntof);
        auto clstime = thiscluster->GetTime();
        auto clstimeraw = thiscluster->GetTimeRaw();
        auto tofchannel = thiscluster->GetTOFchannel();

        Int_t detId_TOF[5];
        Float_t pos_TOF[3];
        AliTOFGeometry::GetVolumeIndices(tofchannel, detId_TOF);
        Float_t x_TOF = AliTOFGeometry::GetX(detId_TOF);
        Float_t y_TOF = AliTOFGeometry::GetY(detId_TOF);
        Float_t z_TOF = AliTOFGeometry::GetZ(detId_TOF);

        TRD_ST_TOF_hit = TRD_ST_Event->createTOFhit();  // TOF hit
        TVector3 TV3_offset(x_TOF, y_TOF, z_TOF);
        TRD_ST_TOF_hit->set_TV3_offset(TV3_offset);
        TRD_ST_TOF_hit->set_time(clstime);

        // printf("ntof: %d, clstime: %4.3f, clstimeraw: %4.3f, pos: {%4.3f, %4.3f, %4.3f} \n",ntof,clstime,clstimeraw,x_TOF,y_TOF,z_TOF);

        // Float_t TOF_Z   = thiscluster->GetZ(); // always 0

        // printf("TOF pos: {%4.3f} \n",TOF_Z);

        /*
        auto hit0idx = thiscluster->GetHitIndex(0);
        AliESDTOFHit* thishit = (AliESDTOFHit*) Arr_TOF_Hits->At(hit0idx);
        auto clsR = thishit->GetR();
        ...
        for (Int_t i_column = 0; i_column < N_columns; i_column++) {
            for (Int_t i_row = 0; i_row < N_rows; i_row++) {
                ...
                for (Int_t i_time = 0; i_time < N_times; i_time++) {
                    ...
                    Int_t fBaseline = fDigMan->GetDigitsParam()->GetADCbaseline(i_det);  // ADC baseline to be subtracted from digit ADC
                                                                                         // values, constant value of 10
                    ADC_amplitude_sum_times += ADC_amplitude;

                    if (ADC_amplitude > 0.0)  // change to 0
                    {
                        // if(
                        // N_total_events == 1 &&
                        //   i_det == 133 && ADC_amplitude > 20.0) // XALEX comment after test
                        //{
                        //    Float_t valuegainiteration = chambergain->GetValue(i_det);
                        //    Float_t GainFactor = KryptoGain ->GetGainCorrectionFactor(i_det,i_row,i_column);

                        // printf("N_total_events: %d, i_det: %d, ADC_amplitude: %4.3f, col: %d, row: %d, time: %d, GainFactor: %4.3f,
                        // chamber gain: %4.3f \n",N_total_events,i_det,ADC_amplitude,i_column,i_row,i_time,GainFactor,valuegainiteration);
                        // // XALEX comment after test
                        //}

                        h_ADC->Fill(ADC_amplitude);
                        ...
                        // Double_t             TRD_drift_time           =
                        // ((Double_t)i_time)*TRD_time_per_bin*ChamberVdrift->GetValue(i_det); // 100 ns per time bin, 1.56 cm/mus drift
                        // velocity, 3 cm drift length at maximum
                        Double_t TRD_drift_time =
                            ((Double_t)i_time) * TRD_time_per_bin *
                            vD_calib;  // 100 ns per time bin, 1.56 cm/mus drift velocity, 3 cm drift length at maximum
                        Double_t TRD_drift_time_uncalib =
                            ((Double_t)i_time) * TRD_time_per_bin *
                            1.56;  // 100 ns per time bin, 1.56 cm/mus drift velocity, 3 cm drift length at maximum
			...
			TM_TRD_rotation_sector[i_sector].LocalToMaster(glb_vec_old,glb_vec_align_sec_old);
                        ...
                        Double_t loc_uncalib[3] = {
                            ...
                            TRD_row_pos - TRD_row_size / 2.0 -
                                padplane->GetRowPos(padplane->GetNrows() / 2)};  // RS: I assume this this correct, depends e.g. on units of
                                                                                 // TRD_time0 and TRD_drift_time
                        Double_t glb_uncalib[3];
                        fGeo->GetClusterMatrix(i_det)->LocalToMaster(
                            loc_uncalib, glb_uncalib);  // this gives tracking coordinates, i.e. lab coordinates rotated to the sector frame
                        ...
                        // uncalibrated
                        Double_t loc[3] = {
                            fGeo->AnodePos() - TRD_drift_time_uncalib, TRD_loc_Y_uncalib,
                            TRD_row_pos - TRD_row_size / 2.0 -
                                padplane->GetRowPos(padplane->GetNrows() / 2)};  // RS: I assume this this correct, depends e.g. on units of
                                                                                 // TRD_time0 and TRD_drift_time
                        Double_t glb[3];
                        fGeo->GetClusterMatrix(i_det)->LocalToMaster(
                            loc, glb);  // this gives tracking coordinates, i.e. lab coordinates rotated to the sector frame
                        ...
                        // Determine global pointing vector of TRD plane (vector perpendicular to plane in global system)  // here no
                        // changes
                        Double_t loc_vec[3] = {
                            fGeo->AnodePos(), 0.0,
                            (TRD_row_end + TRD_row_start) / 2.0 - TRD_row_size / 2.0 - padplane->GetRowPos(padplane->GetNrows() / 2)};
                        Double_t glb_vec[3] = {0.0, 0.0, 0.0};
                        fGeo->GetClusterMatrix(i_det)->LocalToMaster(
                            loc_vec, glb_vec);  // this gives tracking coordinates, i.e. lab coordinates rotated to the sector frame
                        ...
                        Double_t TRD_radius_uncalib =
                            TMath::Sqrt(glb_align_uncalib[0] * glb_align_uncalib[0] + glb_align_uncalib[1] * glb_align_uncalib[1]);
			...
			std::cout << "ADC_amplitude: " << ADC_amplitude << ", i_row: " << i_row
			    ...
			    << ", col_size: " << TRD_col_size << ", row_size: " << TRD_row_size << std::endl;
                        ...
                        if (ADC_amplitude > 0.0) {
                            // printf("i_det: %d, i_column: %d, i_row: %d, i_time: %d, radius: %4.3f, TRD_time0 - TRD_drift_time: %4.3f,
                            ...
                            TV3_TRD_hit_det_angle.SetXYZ(glb_vec_align[0], glb_vec_align[1],
                                                         0.0);  // glb_vec is from {0,0,0} to center of detector, set z component to 0 to
                                                                // get vector perpendicular to detector plane
                ...
                if (ADC_amplitude_sum_times > 0.0 && max_time_bin > 0) {
                    ...
                    Float_t TRD_loc_Y = TRD_col_pos + 0.5 * TRD_col_size;
                    Double_t TRD_drift_time =
                        ((Double_t)(max_time_bin + 0.5)) * TRD_time_per_bin *
                        ChamberVdrift->GetValue(i_det);  // 100 ns per time bin, 1.56 cm/mus drift velocity, 3 cm drift length at maximum
                                                         // Double_t             TRD_drift_time   =
                                                         // ((Double_t)12)*TRD_time_per_bin*ChamberVdrift->GetValue(i_det); // 100 ns per
                                                         // time bin, 1.56 cm/mus drift velocity, 3 cm drift length at maximum
		    ...
		    Double_t glb_vec_ref_align_old[3];
                    ...
                    Double_t glb_ref[3];
                    fGeo->GetClusterMatrix(i_det)->LocalToMaster(
                        loc_ref, glb_ref);  // this gives tracking coordinates, i.e. lab coordinates rotated to the sector frame
                    Double_t glb_ref_align[3];
                    fGeo->RotateBack(i_det, glb_ref, glb_ref_align);

                    Double_t loc_vec_ref[3] = {
                        fGeo->AnodePos(), 0.0,
                        (TRD_row_end + TRD_row_start) / 2.0 - TRD_row_size / 2.0 -
                            padplane->GetRowPos(padplane->GetNrows() / 2)};  // RS: I assume this this correct, depends e.g. on units of
                                                                             // TRD_time0 and TRD_drift_time
                    Double_t glb_vec_ref[3];
                    fGeo->GetClusterMatrix(i_det)->LocalToMaster(
                        loc_vec_ref, glb_vec_ref);  // this gives tracking coordinates, i.e. lab coordinates rotated to the sector frame
                    ...
                    TV3_TRD_hit_det_angle_middle.SetXYZ(glb_vec_ref_align[0], glb_vec_ref_align[1],
                                                        0.0);  // glb_vec is from {0,0,0} to center of detector, set z component to 0 to get
                                                               // vector perpendicular to detector plane
                    ...
                    // for(Int_t i_time = 0; i_time < 2; i_time++)
                    {
                        ...
                        AS_Digit->set_pos(i_time, vec_points_time_bins_uncalib[i_time].X(), vec_points_time_bins_uncalib[i_time].Y(),
                                          vec_points_time_bins_uncalib[i_time].Z());
                        // if(i_time == 0)
                        //{
    ...
    for (Int_t iTracks = 0; iTracks < N_tracks; iTracks++) {
        ...
        Long64_t TRD_layer_info[6];  // each value stores all 8 time slices, Long64_t has 8 byte = 8*8 bit = 64 bit in total -> one byte = 8
                                     // bit = 256 per time bin
        memset(TRD_layer_info, 0, sizeof(TRD_layer_info));  // for automatically-allocated arrays

        // Helix
        FillHelix(track, magF);

        AS_Track->setHelix(aliHelix.fHelix[0], aliHelix.fHelix[1], aliHelix.fHelix[2], aliHelix.fHelix[3], aliHelix.fHelix[4],
                           aliHelix.fHelix[5], aliHelix.fHelix[6], aliHelix.fHelix[7], aliHelix.fHelix[8]);
        AS_Track->setHelix_TRD(aliHelix_TRD.fHelix[0], aliHelix_TRD.fHelix[1], aliHelix_TRD.fHelix[2], aliHelix_TRD.fHelix[3],
                               aliHelix_TRD.fHelix[4], aliHelix_TRD.fHelix[5], aliHelix_TRD.fHelix[6], aliHelix_TRD.fHelix[7],
                               aliHelix_TRD.fHelix[8]);
    ...
    for (Int_t i_det = 0; i_det < 540; i_det++) {
        ...
        for (Int_t i_trkl = 0; i_trkl < (Int_t)vec_self_tracklet_fit_points[i_det].size(); i_trkl++) {
            ...
            //     \n",N_good_events,N_tracklets_store,TV3_trkl_offset.X(),TV3_trkl_offset.Y(),TV3_trkl_offset.Z()); printf("  --> pos:
            //     {%4.3f, %4.3f, %4.3f}
    ...
    for (UShort_t i_track = 0; i_track < NumTracks; ++i_track)  // loop over all tracks of the actual event
    {
        ...
        for (Int_t i_helix_par = 0; i_helix_par < 9; i_helix_par++) {
            ...
            helix_TRD_par[i_helix_par] = AS_Track->getHelix_TRD_param(i_helix_par);
        }

        // TRD_ST_TPC_Track
        // ->setHelix(helix_par[0],helix_par[1],helix_par[2],helix_par[3],helix_par[4],helix_par[5],helix_par[6],helix_par[7],helix_par[8]);
        TRD_ST_TPC_Track->setHelix(helix_par[0], helix_par[1], helix_par[2], helix_par[3], helix_par[4], helix_par[5]);
        TRD_ST_TPC_Track->setHelix_TRD(helix_TRD_par[0], helix_TRD_par[1], helix_TRD_par[2], helix_TRD_par[3], helix_TRD_par[4],
                                       helix_TRD_par[5]);
    }
...
Double_t Ali_make_tracklets_from_digits::Calc_SVD_tracklet(Int_t i_det, Int_t i_trkl) {
    ...
    for (Int_t i_time = 0; i_time < (Int_t)vec_connected_clusters[i_det][i_trkl].size(); i_time++) {
        TV3_point.SetXYZ(vec_connected_clusters[i_det][i_trkl][i_time][0], vec_connected_clusters[i_det][i_trkl][i_time][1],
                         vec_connected_clusters[i_det][i_trkl][i_time][2]);
        TV3_mean += TV3_point;
    ...
    for (Int_t i_point = 0; i_point < number_ok_clusters; i_point++) {
        TVector3 testpoint =
            calculate_point_on_Straight_dca_to_Point_2D(TV3_SVD_tracklet_offset, TV3_SVD_tracklet_dir, arr_TV3_points[i_point]);

        Double_t dist_DCA_XY = TMath::Sqrt(TMath::Power(arr_TV3_points[i_point][0] - testpoint[0], 2) +
                                           TMath::Power(arr_TV3_points[i_point][1] - testpoint[1], 2));
...
void Ali_make_tracklets_from_digits::Make_clusters_and_get_tracklets_fit(Double_t Delta_x, Double_t Delta_z, Double_t factor_missing) {
    ...
    for (Int_t i_det = 0; i_det < 540; i_det++) {
        ...
        for (Int_t i_time = 0; i_time < N_time_bins; i_time++) {
            ...
            std::sort(vec_all_TRD_digits[i_det][i_time].begin(), vec_all_TRD_digits[i_det][i_time].end(),
                      sortcol_first);  // large values to small values, last column sorted via function sortcol
            ...
            for (Int_t i_digit_max = 0; i_digit_max < ((Int_t)vec_all_TRD_digits[i_det][i_time].size()); i_digit_max++) {
                ...
                Double_t pos_ADC_max[4] = {
                    vec_all_TRD_digits[i_det][i_time][i_digit_max][0], vec_all_TRD_digits[i_det][i_time][i_digit_max][1],
                    vec_all_TRD_digits[i_det][i_time][i_digit_max][2], vec_all_TRD_digits[i_det][i_time][i_digit_max][3]};
                ...
                for (Int_t i_digit_sub = 0; i_digit_sub < (Int_t)vec_all_TRD_digits[i_det][i_time].size(); i_digit_sub++) {
                    ...
                    Double_t pos_ADC_sub[4] = {
                        vec_all_TRD_digits[i_det][i_time][i_digit_sub][0], vec_all_TRD_digits[i_det][i_time][i_digit_sub][1],
                        vec_all_TRD_digits[i_det][i_time][i_digit_sub][2], vec_all_TRD_digits[i_det][i_time][i_digit_sub][3]};
                    Double_t dist_digits_XY =
                        TMath::Sqrt(TMath::Power(pos_ADC_max[0] - pos_ADC_sub[0], 2) + TMath::Power(pos_ADC_max[1] - pos_ADC_sub[1], 2));
            ...
            for (Int_t i_digit_max = 0; i_digit_max < ((Int_t)vec_all_TRD_digits[i_det][i_time].size() - 1); i_digit_max++) {
                ...
                Double_t pos_ADC_max[4] = {
                    vec_all_TRD_digits[i_det][i_time][i_digit_max][0], vec_all_TRD_digits[i_det][i_time][i_digit_max][1],
                    vec_all_TRD_digits[i_det][i_time][i_digit_max][2], vec_all_TRD_digits[i_det][i_time][i_digit_max][3] - baseline};
                Int_t N_digits_added = 1;
                Double_t Sum_ADC_weight = pos_ADC_max[3];  // ADC as weight
                if (Sum_ADC_weight < 0.0) continue;
                Double_t pos_ADC_sum[4] = {pos_ADC_max[0] * pos_ADC_max[3], pos_ADC_max[1] * pos_ADC_max[3],
                                           pos_ADC_max[2] * pos_ADC_max[3], pos_ADC_max[3]};  // positions times ADC value [3]
                ...
                for (Int_t i_digit_sub = (i_digit_max + 1); i_digit_sub < (Int_t)vec_all_TRD_digits[i_det][i_time].size(); i_digit_sub++) {
                    ...
                    Double_t pos_ADC_sub[4] = {
                        vec_all_TRD_digits[i_det][i_time][i_digit_sub][0], vec_all_TRD_digits[i_det][i_time][i_digit_sub][1],
                        vec_all_TRD_digits[i_det][i_time][i_digit_sub][2], vec_all_TRD_digits[i_det][i_time][i_digit_sub][3] - baseline};
    ...
    for (Int_t i_det = 0; i_det < 540; i_det++)  // is done chamber wise
    {
        ...
        // for(Int_t i_time = 0; i_time < 1; i_time++) // is done chamber wise  ALEX
        {
            ...
            std::sort(vec_all_TRD_digits_clusters[i_det][i_time].begin(), vec_all_TRD_digits_clusters[i_det][i_time].end(),
                      sortcol_first);  // large values to small values, last column sorted via function sortcol
            ...
            for (Int_t i_cls = 0; i_cls < N_clusters; i_cls++)  // loop over all clusters in one detector and for time bin 0
            {
                ...
                Double_t pos_ADC_max[4] = {
                    vec_all_TRD_digits_clusters[i_det][i_time][i_cls][0], vec_all_TRD_digits_clusters[i_det][i_time][i_cls][1],
                    vec_all_TRD_digits_clusters[i_det][i_time][i_cls][2], vec_all_TRD_digits_clusters[i_det][i_time][i_cls][3]};

                Int_t N_digits_used = vec_all_TRD_digits_clusters[i_det][i_time][i_cls][4];
                if (N_digits_used <= 1) continue;

                Double_t radius = TMath::Sqrt(TMath::Power(vec_all_TRD_digits_clusters[i_det][i_time][i_cls][0], 2) +
                                              TMath::Power(vec_all_TRD_digits_clusters[i_det][i_time][i_cls][1], 2));

                // if(i_time == 0) printf("---> i_det: %d, i_cls: %d, time 0, pos: {%4.3f, %4.3f, %4.3f}, radius: %4.3f, ADC: %4.3f,
                // N_digits_used: %d
                ...
                for (Int_t i_time_sub = i_time_start; i_time_sub < N_time_bins; i_time_sub++) {
                    ...
                    for (Int_t i_cls_sub = 0; i_cls_sub < N_clusters_sub; i_cls_sub++) {
                        ...
                        Double_t pos_ADC_sub[4] = {vec_all_TRD_digits_clusters[i_det][i_time_sub][i_cls_sub][0],
                                                   vec_all_TRD_digits_clusters[i_det][i_time_sub][i_cls_sub][1],
                                                   vec_all_TRD_digits_clusters[i_det][i_time_sub][i_cls_sub][2],
                                                   vec_all_TRD_digits_clusters[i_det][i_time_sub][i_cls_sub][3]};

                        Double_t dist_clusters_XY = TMath::Sqrt(TMath::Power(pos_ADC_max[0] - pos_ADC_sub[0], 2) +
                                                                TMath::Power(pos_ADC_max[1] - pos_ADC_sub[1], 2));
                    ...
                    for (Int_t i_xyzADC = 0; i_xyzADC < 4; i_xyzADC++) {
                        pos_ADC_max[i_xyzADC] =
                            vec_all_TRD_digits_clusters[i_det][i_time_sub][best_sub_cluster][i_xyzADC];  // The one connected is now the new
                                                                                                         // "first" one for the next search
                                                                                                         // step in time
                        vec_single_point[i_xyzADC] = pos_ADC_max[i_xyzADC];
                    }
    ...
    for (Int_t i_det = 0; i_det < 540; i_det++) {
        ...
        for (Int_t i_trkl = 0; i_trkl < (Int_t)vec_connected_clusters[i_det].size(); i_trkl++) {
            ...
            // Double_t radius_in  = TMath::Sqrt( TMath::Power(vec_connected_clusters[i_det][i_trkl][tbn_max-1][0],2) +
            // TMath::Power(vec_connected_clusters[i_det][i_trkl][tbn_max-1][1],2) ); Double_t radius_out = TMath::Sqrt(
            // TMath::Power(vec_connected_clusters[i_det][i_trkl][0][0],2) + TMath::Power(vec_connected_clusters[i_det][i_trkl][0][1],2) );
...
void Ali_make_tracklets_from_digits::FillHelix(AliESDtrack* track_in, Double_t magF_in) {
    ...
    for (Int_t i_ext_params = 0; i_ext_params < 2; i_ext_params++)  // loop over different helix parameters
    {
        ...
        if (i_ext_params == 1) {
            ...
            alpha_alt = alpha_TRD;
            for (Int_t i_param = 0; i_param < 5; i_param++) {
                fHelix_alt[i_param] = p_param_TRD[i_param];
            }
        }
        ...
        if (i_ext_params == 1) {
            for (Int_t i_param = 0; i_param < 9; i_param++) {
                aliHelix_TRD.fHelix[i_param] = fHelix_alt[i_param];
            }
        }
    }
...
//________________________________________________________________________
void Ali_make_tracklets_from_digits::FindDCAHelixPoint(TVector3 space_vec, AliHelix helixA, Float_t path_initA, Float_t path_initB,
